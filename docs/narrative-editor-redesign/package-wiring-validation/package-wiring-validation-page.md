...
- 当前整个 package 是否健康
- 哪些后台无法自动化收敛的问题仍然存在
- 哪些问题已经超出当前 section 页右下角 coordinator 的局部修复范围
- 哪些问题是阻塞项
- 应该回哪一页修
...
- 问题来源
- 影响范围
- 为什么这一项没有在局部 coordinator 中被静默解决
- 建议修复入口
- 当前状态类型
...
右下继续保留 coordinator，但它在本页的角色和前三页不同。

在本页里，coordinator 应更偏向：
...
默认情况下，本页也不应抢走各 section 页右下 coordinator 的问题解释职责。

## 8. Coordinator Relation

本页和前三页右下角的 coordinator 不是重复关系。

正确分工是：

- 各 section 页右下 coordinator：
  - 负责局部编辑反馈
  - 解释当前页这次提交的问题
  - 尽可能自动解决当前页的大多数可修复问题
...
4. 右下 coordinator 要偏全局诊断，不要复刻前三页的局部填写逻辑
5. 本页应以“引导作者回正确 section 修复”为核心，而不是把修复动作全部搬到本页
6. 后台检查逻辑属于系统能力，本页只负责展示和引导

## 11. Pending Pairing Note

本页先于本 section 的 skill / family 文档落地。

如果后续这个 section 仍保留为独立 section，而不是完全降为后台能力，那么对应的 skill 设计必须与本页对齐，尤其要对齐：

- 本页是全局诊断面，不是内容编辑页
- 本页展示的是后台自动检查结果
- 本页的核心价值是“看清问题并跳回正确入口修”

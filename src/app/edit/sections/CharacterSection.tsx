...
export interface CharacterSectionProps {
  ...
  readonly selection?: CharacterSelection;
  readonly onSelectionChange?: (nextSelection: CharacterSelection) => void;
...
function summarizeSecondaryLine(character: WorldBaseCharacterDraft): string {
  const parts = [character.gender.trim(), character.personality.trim()].filter((entry) => entry.length > 0);
...
export function CharacterSection({
  ...
  const [localSelection, setLocalSelection] = useState<CharacterSelection>({ group: 'hero' });
  ...
  function setSelection(nextSelection: CharacterSelection) {
  ...
  useEffect(() => {
    ...
    onSelectionChange?.({ group: 'hero' });
    if (!isSelectionControlled) {
      setLocalSelection({ group: 'hero' });
    }
  }, [

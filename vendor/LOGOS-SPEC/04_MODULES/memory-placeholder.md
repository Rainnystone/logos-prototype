...
1. 提供最近 5 个 Beat 的可读窗口。
2. 在不足 5 个 Beat 时返回全部已有历史。
...
把记忆先作为 placeholder 单列出来，有两个工程上的好处。第一，它能防止“最近 5 Beat 读取”在多个模块中散落成硬编码规则。第二，它给未来完整记忆模块留出了自然扩展点，而不会逼迫系统在后期进行大范围架构迁移。
...
当前 Sample 明确采用“直接读取最近 5 个 Beat”的策略，这条规则必须只存在于这个模块和编排层中，而不应被复制到系统的其他角落。后续第五阶段提炼契约时，也应围绕这一占位模块来抽出 `precedingBeats` 的结构，而不是在多个模块里各自定义一版。

...
> - 完整运行循环的权威源仍是 `LOGOS-SPEC/03_ORCHESTRATION/` 下的编排层文档；本文只从 API 视角引用，不再复写一份完整编排流程。
...
本节只保留 API Adapter Lite 在一个 Beat 周期中的介入点与调用次数。完整的叙事推进、重写裁决、Phase/Scene 状态流转，以及是否触发记忆压缩，仍然以 `LOGOS-SPEC/03_ORCHESTRATION/` 下的编排层文档为权威源；本文不再复写一份并行流程，以避免仓库内外同时存在两套看起来都像主规范的运行图。
